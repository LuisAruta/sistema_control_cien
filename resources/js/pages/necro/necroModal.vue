<template>
  <b-form @submit.stop.prevent="handleSubmit">
    ...
    <b-modal id="necroModal" header-class="border-bottom-0" footer-class="border-top-0" :title="tituloModal" modal size="xl" @ok="handleOk">
            ...
            <b-col>
              ...
              >
                <b-row v-if="allowEdit" align-v="center" style="margin-bottom: 10px;">
                  ...
                  <b-col sm="9">
                    <b-form-input v-model="necro.numero_de_registro"
                                  :class="{ 'is-invalid': $v.necro.numero_de_registro.$error }"
                                  placeholder="Ingrese número de registro"
                    />
                  </b-col>
                </b-row>
                <b-row v-if="$v.necro && $v.necro.numero_de_registro.$error" align-h="end"
                       align-v="center"
                >
                  <b-col class="text-danger" sm="9">
                      <span v-if="!this.$v.necro.numero_de_registro.required">El número de registro es requerido</span>
                      <span v-else-if="!this.$v.necro.numero_de_registro.minValue || !this.$v.necro.numero_de_registro.maxValue">
                ...
                <b-row align-v="center">
                  ...
                  <b-col sm="9">
                    <b-form-input v-model="necro.legajo_cuerpo_medico_forense"
                                  :class="{ 'is-invalid': $v.necro.legajo_cuerpo_medico_forense.$error }"
                                  placeholder="Ingrese el legajo"
                    />
                  </b-col>
                </b-row>
                <b-row v-if="$v.necro && $v.necro.legajo_cuerpo_medico_forense.$error" align-h="end"
                       align-v="center"
                >
                  <b-col sm="9" class="text-danger">
                    <span v-if="!this.$v.necro.legajo_cuerpo_medico_forense.required">El Legajo es requerido</span>
                    <span v-else>El legajo debe tener entre {{
                      $v.necro.legajo_cuerpo_medico_forense.$params.minLength.min
                    }} a {{ $v.necro.legajo_cuerpo_medico_forense.$params.maxLength.max }} cifras</span>
                  </b-col>
                </b-row>
                <br>
                <b-row align-v="center">
                  ...
                  <b-col sm="9">
                    <b-form-select v-model="necro.delegacion_dependencia_id"
                                   :class="{ 'is-invalid': $v.necro.delegacion_dependencia_id.$error }"
                                   :options="delegaciones"
                                   :reduce="delegacion => delegacion.id"
                                   text-field="nombre" value-field="id"
                                   label="nombre" placeholder="Seleccione una dependencia"
                    />
                  </b-col>
                </b-row>
                <b-row v-if="$v.necro && $v.necro.delegacion_dependencia_id.$error" align-h="end"
                       align-v="center"
                >
                  <b-col class="text-danger" sm="9">
                ...
                <b-row align-v="center">
                  ...
                  <b-col sm="9">
                    <b-form-select v-model="necro.interviniento_dependencia_id"
                                   :class="{ 'is-invalid': $v.necro.interviniento_dependencia_id.$error }"
                ...
                <b-row v-if="$v.necro && $v.necro.interviniento_dependencia_id.$error" align-h="end"
                       align-v="center"
                >
                  <b-col class="text-danger" sm="9">
                ...
                <b-row align-v="center">
                  ...
                  <b-col sm="9">
                    <b-form-select v-model="necro.perito_usuario_id"
                                   :class="{ 'is-invalid': $v.necro.perito_usuario_id.$error }" :options="perito_usuarios"
                ...
                <b-row v-if="$v.necro && $v.necro.perito_usuario_id.$error" align-h="end" align-v="center">
                  <b-col class="text-danger" sm="9">
                ...
                <div v-if="allowEdit">
                  ...
                  <b-row align-v="center">
                    ...
                    <b-col sm="9">
                      <b-form-select v-model="necro.estado_id" :options="estados" value-field="id"
                                     text-field="nombre"
                      >
                        <template #first>
                  ...
                  <b-row v-if="allowEdit && $v.necro && $v.necro.estado_id.$error" class="text-danger"
                         align-h="end"
                  >
                    <b-col sm="9">
      ...
      <template slot="modal-footer">
        ...
        <b-button type="button" @click="$bvModal.hide('necroModal')">
          Cancelar
        </b-button>
      </template>
    </b-modal>
...
<script>
import { minLength, maxLength, minValue, maxValue, numeric, required } from 'vuelidate/lib/validators'
...
export default {
  ...
  mounted () {
    this.$root.$on('bv::modal::shown', (bvEvent, modalId) => {
      if (modalId === 'necroModal') {
        this.cleanForm()
        if (this.row && this.row.id) {
          this.allowEdit = true
          this.tituloModal = 'Editar Necro'
          this.necro.id = this.row.id
          this.necro.numero_de_registro = this.row.numero_de_registro
          this.setearHorayFecha(this.row.fecha_hora)
          this.lugar = this.row.lugar
          this.necro.lugar_id = this.row.lugar.id
          this.necro.legajo_cuerpo_medico_forense = this.row.legajo_cuerpo_medico_forense
          ...
          this.necro.delegacion_dependencia_id = this.row.delegacion_dependencia.id
          this.necro.interviniento_dependencia_id = this.row.interviniento_dependencia.id
          this.necro.perito_usuario_id = this.row.perito_usuario.id
          this.necro.estado_id = this.row.estado.id
        } else {
          this.allowEdit = false
          this.tituloModal = 'Nuevo Necro'
          if (this.necro.delegacion_dependencia_id === null) {
            this.necro.delegacion_dependencia_id = this.user.dependencia_id
          }
  ...
  methods: {
    cleanForm () {
      ...
      this.max = null
      this.necro.id = ''
      this.necro.numero_de_registro = 1
      this.necro.fecha_hora = ''
      this.necro.expediente_id = null
      this.necro.perito_usuario_id = null
      this.necro.delegacion_dependencia_id = null
      this.necro.interviniento_dependencia_id = null
      this.necro.legajo_cuerpo_medico_forense = null
      this.necro.lugar_id = null
      this.necro.estado_id = 1
      this.$v.$reset()
    },
    cleanDatosCadaverExpediente () {
      this.datosCadaver = {}
      this.datosCadaver.foto = false
      this.datosCadaver.identificado = true
      this.datosCadaver.cadaver_nombre = ''
    ...
    setearHorayFecha (fechahora) {
      ...
      if (!this.permissions.includes('crear_necro_viejo') && !this.permissions.includes('editar_necro_viejo')) { // Si no tiene ninguno de estos permisos se setea el minimo
    ...
    modificarOficios (event) {
      ...
      axios.get('api/oficio/searchBy/necro/' + this.necro.id)
        .then((result) => {
    ...
    submitEditar () {
      this.necro.fecha_hora = this.fecha_hora
      axios.put('api/necro/' + this.necro.id, {
        necro: this.necro,
        lugar: this.lugar,
        ...
        this.$nextTick(() => {
          this.mostrarSpinner(false)
          this.$emit('cambios-array-necro')
          this.$bvModal.hide('necroModal')
        })
      }).catch(error => {
        this.mostrarSpinner(false)
