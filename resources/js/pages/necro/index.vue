<template>
  <b-container fluid style="padding-left: 0px !important; padding-right: 0px !important;">
    <necro-modal :row="selectedRow"
                 ...
                 @cambios-array-necro="getData()" />
    <detalle-modal :info-modal="infoModal" @close="resetInfoModal"></detalle-modal>
    ...
    <b-card style="background-color: transparent;">
      <b-row>
        ...
        <b-col lg="2" style="text-align: end;">
          <b-button
            v-if="permissions.includes('crear_necro')"
            size="sm"
            type="button"
            variant="primary"
    ...
    >
      <template #cell(acciones)="row">
        <b-button v-if="permissions.includes('consultar_necro')" v-b-tooltip.hover="'Ver Detalles'" size="sm"
                  variant="info" @click="abrirModalDetalle(row.item)"
        >
          <b-icon aria-hidden="true" icon="search" />
        </b-button>
        <b-button
          v-if="permissions.includes('editar_necro_viejo') || (row.item.editable && permissions.includes('editar_necro'))"
        ...
        <b-button
          v-if="permissions.includes('eliminar_necro')"
...
import necroModal from './necroModal'
...
export default {
  name: 'NecroVue',
  components: { oficioModal, necroModal, detalleModal },
  middleware: 'auth',
  data () {
    return {
      isLoading: false,
      url: 'api/necro/search',
      total: 0,
      modal: necroModal,
      fields: [
        {
  ...
  methods: {
    ...
    abrirModalOficio (data) {
      ...
      this.selectedRow.tipo = 'Necro'
      this.$bvModal.show('oficioModal')
    },
    abrirModalDetalle (item) {
      this.infoModal.title = 'Detalles Necro'
    ...
    eliminarRow (event, row) {
      Swal.fire({
        title: '¿Desea eliminar la Necro?',
        text: 'Eliminar una Necro es permanente.',
        icon: 'warning',
        ...
        if (result.value) {
          this.mostrarSpinner(true)
          axios.delete('api/necro/' + row.id)
            .then(response => {
              this.$bvToast.toast('La Necro fue borrada correctamente', {
                title: 'Exito',
                autoHideDelay: 3000
              })
              this.getData()
